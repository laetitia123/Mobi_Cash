...
def gen_pdf(request):
    ...
    name= Project.objects.filter(pk=cid).select_related('user')
    params = {
        'name': name,
        'request': request,
        'project':project
    }
    return Render.render('mobicashapp/pdf.html', params)
    # ......................to export in xls.................
def export_xls(request):
    response = HttpResponse(content_type='application/ms-excel')
    response['Content-Disposition'] = 'attachment; filename="mobicash.xls"'
    ...
    columns = ['name', 'adress', 'phone']
    for col_num in range(len(columns)):
        ws.write(row_num, col_num, columns[col_num], font_style)
    # Sheet body, remaining rows
    font_style = xlwt.XFStyle()
    rows = Project.objects.all().values_list('name', 'adress',   'phone')
    for row in rows:
        row_num += 1
